...
export default function Sys02UserPage({ companyId: propCompanyId }: { companyId?: number }) {
  ...
  return (
    <div className={`flex h-full gap-3 ${propCompanyId ? 'p-0' : 'p-3'}`}>
      {!propCompanyId && (
      ...
      {propCompanyId && (
        <div className="w-64 flex flex-col bg-white border-r border-gray-200 overflow-hidden shrink-0">
          <div className="flex items-center justify-between px-3 py-2 bg-gray-50 border-b border-gray-200">
             <span className="text-xs font-bold text-gray-700">사용자 목록</span>
             <button onClick={handleNew} className="p-1 bg-green-600 text-white rounded hover:bg-green-700">
                <Plus size={12} />
             </button>
          </div>
          <div className="flex-1 overflow-y-auto">
             {users.map(u => (
                <button key={u.userId} onClick={() => handleSelect(u)}
                  className={`w-full text-left px-3 py-2 border-b border-gray-100 hover:bg-gray-50 transition-colors ${selected?.userId === u.userId ? 'bg-green-50' : ''}`}>
                  <div className="text-xs font-medium text-gray-800">{u.korNm}</div>
                  <div className="text-[10px] text-gray-400">{u.loginId}</div>
                </button>
             ))}
          </div>
        </div>
