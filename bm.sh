...
print_banner() {
    ...
    echo -e "${CYAN}${BOLD}   ╚═════╝ ╚═╝     ╚═╝╚══════╝${NC}"
    echo -e ""
    echo -e "  ${DIM}Backend Manager${NC}  ${BOLD}v${VERSION}${NC}  ${DIM}│${NC}  mode: ${YELLOW}${BOLD}${OMS_MODE:-?}${NC}"
    echo -e "  ${DIM}──────────────────────────────────────────────────────────${NC}"
    echo -e ""
}
...
resolve_mode() {
    ...
    echo -e "\n${BOLD}OMS_MODE 환경변수가 설정되지 않았습니다.${NC}"
    ...
    export OMS_MODE="${modes[$((choice-1))]}"
    info "모드 설정: ${OMS_MODE}"
    echo -e "${YELLOW}[TIP]${NC}  다음부터 자동 적용하려면: ${BOLD}export OMS_MODE=${OMS_MODE}${NC}"
    echo ""
}

load_env() {
    local env_file="$SCRIPT_DIR/.env.${OMS_MODE}"
    if [[ ! -f "$env_file" ]]; then
...
# ── 명령어 함수 ──────────────────────────────────────────────────────────
do_run() {
    header "Run - 개발 서버 실행 (mode: ${OMS_MODE})"
    load_env
    if [[ ! -f "$GRADLEW" ]]; then
    ...
    info "Health check: curl http://localhost:8686/oms/auth/health"
    echo ""
    "$GRADLEW" -p "$BACKEND_DIR" bootRun --args="--spring.profiles.active=${OMS_MODE}"
}
...
do_log() {
    local log_file="$BACKEND_DIR/oms-data/logs/oms.log"
